...
const Weather = () => {
  ...
  const getBackground = (main) => {
    ...
    if (lowerMain.includes("cloud") || lowerMain.includes("mist") || lowerMain.includes("fog") || lowerMain.includes("haze")) return cloudy_bg;
  ...
  const search = async (city) => {
    ...
    if (!import.meta.env.VITE_APP_ID) {
      alert("API key is missing. Please add VITE_APP_ID to your .env file");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Get unique days from forecast data (API provides 5 days)
      const dailyForecast = forecastData.list
        .filter((item, index, arr) => {
          const date = new Date(item.dt_txt).toDateString();
          return arr.findIndex(i => new Date(i.dt_txt).toDateString() === date) === index;
        })
        .slice(0, 5)
        .map(day => ({
  ...
  const backgroundImage = getBackground(weatherData?.main);
  ...
  return (
    <div
      ...
      style={{
        ...
        filter: isDay ? 'brightness(1)' : 'brightness(0.6)',
      }}
    >
      {!isLoggedIn ? (
